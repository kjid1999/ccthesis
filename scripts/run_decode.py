...
if __name__ == '__main__':
    ...
    parser.add_argument('--sel_ckpt', type=str, default='last', help='select which checkpoints are used in sampling')
    parser.add_argument('--see_every_step', type=bool, default=False, help='If true, will only decode one sentence, and store its every x0.')
    ...
    output_lst = []
    # for lst in glob.glob(args.model_dir)[-1:]: # 用train最久的model
    for lst in glob.glob(args.model_dir):
        print(lst)
        # checkpoints = sorted(glob.glob(f"{lst}/{args.pattern}*pt"))[::-1]
        if args.sel_ckpt == 'last':
            checkpoints = sorted(glob.glob(f"{lst}/{args.pattern}*pt"))[::-1][:1] # last
        elif args.sel_ckpt == 'all':
            checkpoints = sorted(glob.glob(f"{lst}/{args.pattern}*pt"))[::-1] # all
        else:
            checkpoints = glob.glob(f"{lst}/{args.pattern}*{args.sel_ckpt}*pt")
