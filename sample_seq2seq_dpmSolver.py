...
def main():
    ...
    # for cond in tqdm(all_test_data[:1]): # I modified, run one data for debug
        ...
        # print(noise.shape)
        # print(model.mean_embed.shape)
        # exit()
        ## You can use steps = 10, 12, 15, 20, 25, 50, 100.
        ## Empirically, we find that steps in [10, 20] can generate quite good samples.
        ## And steps = 20 can almost converge.
        with autocast():
            x_sample, intermediate= dpm_solver.sample(
                x_noised,
                steps=SOLVER_STEP,
                order=2,
                ...
                x_start=x_start,
                return_intermediate=True # I add on
            )
        # print(x_sample[0].shape) # samples for each step [128, 128]

        # see sampling process
        # =========
        # intermediate_tensors = torch.stack(intermediate)[:, 0]
        # pad_len = x_sample.shape[0] - intermediate_tensors.shape[0]
        # pad_shape = intermediate_tensors.shape[1:]
        # pad = torch.zeros((pad_len, *pad_shape)).cuda()
        # x_sample = torch.cat((intermediate_tensors, pad))
